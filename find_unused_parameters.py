...
def getCommandLineArgs():
    ...
    parser.add_argument("schema_file", help="Path to schema.")
    parser.add_argument("directory", help="Directory where the configuration file(s) to be converted are located.")
    parser.add_argument("file_name", nargs='?', default="config.json, param_overrides.json,campaign.json", help="filename for file(s) that should be converted. With -r all files that end with file_name (e.g. config.json converts test_config.json) are found as well. (default = config.json)")
...
if __name__ == '__main__':
    ...
    name_config_file = commandline_args.file_name.split(',')
    ...
    if commandline_args.r:
        ...
        dirs = [os.path.join(WorkingDir, n) for n in name_config_file]

    with open(commandline_args.schema_file, 'r+') as f:
        try:
            schema = json.load(f)
