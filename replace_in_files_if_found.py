'''
this script provides some functions that return a list of directories from
- an xml file (usually generated by build server)
- a json file (usually generated by build server)

- list of paths to files with some name in a directory (looks in subdirectories for the file)
- a more general function to look for files in subdirs
'''
...
replace_this = "Base_Incubation_Period"
...
#use directory
WorkingDir = os.path.normcase(r"C:\Users\tfischle\GitHub\DtkTrunk\Regression")
...
for t in dirs:
    ...
    try:
        with open(campaign_file, 'r') as f:
            ...
            if found_fct(content, "\"Incubation_Period_Distribution\": \"FIXED_DURATION\""):
                with_that = "Incubation_Period_Fixed"
            elif found_fct(content, "\"Incubation_Period_Distribution\": \"EXPONENTIAL_DURATION\""):
